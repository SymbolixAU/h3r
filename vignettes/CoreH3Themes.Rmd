...
library(h3r)
```
### Resolution 0 cells and pentagons
You can see that all of the earth is covered with those cells. Note that the size of cells far from the equator are exaggerates becauses of the use of The Mercator projection. All cells are of roughly the same size.
...
```
Note that according to uber "The H3 grid is constructed on the icosahedron by recursively creating increasingly higher precision hexagon grids until the desired resolution is achieved. Note that it is impossible to tile the sphere/icosahedron completely with hexagons; each resolution of an icosahedral hexagon grid must contain exactly 12 pentagons at every resolution, with one pentagon centered on each of the icosahedron vertices". Therefore, not all cells are hexagon: some are pentagons. Let's see what are the pentagons in resolution 0.
```{r}
mapdeck::set_token(secret::get_secret("MAPBOX"))
